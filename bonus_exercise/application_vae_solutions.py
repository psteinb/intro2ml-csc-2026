...
def _():
    ...
    return F, device, nn, np, plt, torch, torchvision
...
def _(F, nn, torch):
    ...
    class Encoder(nn.Module):
        def __init__(self, latent_dims):
            ...
            # YOUR CODE HERE: Define a linear layer from 784 to 512 neurons
            self.linear1 = nn.Linear(784, 512)
            # YOUR CODE HERE: Define a linear layer from 512 to the latent dimension
            self.linear2 = nn.Linear(512, latent_dims)

        def forward(self, x):
            # The forward pass defines how data flows through the layers.
            # First, we flatten the 28x28 image into a 784-dim vector.
            x = torch.flatten(x, start_dim=1)
            # We apply a ReLU activation function to the first layer's output.
            x = F.relu(self.linear1(x))
            # The final output is the latent vector.
...
def _(F, nn, torch):
    ...
    class Decoder(nn.Module):
        def __init__(self, latent_dims):
            ...
            # YOUR CODE HERE: Define a linear layer from the latent dimension to 512 neurons
            self.linear1 = nn.Linear(latent_dims, 512)
            # YOUR CODE HERE: Define a linear layer from 512 to 784 neurons
            self.linear2 = nn.Linear(512, 784)

        def forward(self, z):
            # We apply a ReLU activation function.
            z = F.relu(self.linear1(z))
            # We use a Sigmoid activation on the final layer. Why?
            # Because the input pixel values are normalized between 0 and 1.
            # Sigmoid squashes the output to be in the same range.
    ...
    def train(autoencoder, data, epochs=20):
        ...
        for epoch in range(epochs):
            for x, y in data:
                ...
                # Hint: The formula is (x - x_hat)^2, and you can sum it up.
                loss = ((x - x_hat)**2).sum()
...
def _(Decoder, F, device, nn, torch):
    ...
    class VariationalEncoder(nn.Module):
        def __init__(self, latent_dims):
            ...
            # YOUR CODE HERE: The VAE encoder needs two output layers,
            ...
            self.linear_log_sigma = nn.Linear(512, latent_dims)
        ...
        def forward(self, x):
            ...
            mu =  self.linear_mu(x)
            # We use exp to ensure sigma is always positive
            sigma = torch.exp(self.linear_log_sigma(x))
        
            # YOUR CODE HERE: Sample the latent vector `z` from the distribution N(mu, sigma).
            # This is the "reparameterization trick": z = mu + sigma * (a random sample from N(0,1))
            z = mu + sigma * self.N.sample(mu.shape)
        
            # YOUR CODE HERE: Calculate the KL divergence.
            # The formula is (sigma^2 + mu^2 - log(sigma) - 1/2). Sum it over all dimensions.
